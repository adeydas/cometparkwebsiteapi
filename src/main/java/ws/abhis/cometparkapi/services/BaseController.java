...
public class BaseController {
	...
	public String searchSpaces(ModelMap model, @PathVariable String lotId) {
		...
		centroidLatLon = objDatabaseConnect
				.runSqlQuery("select lat, lon from tbl_parkinglots where lotid='"
						+ lotId + "'");
		...
		spacesList = objDatabaseConnect
				.runSqlQuery("select id, lat, lon, leveltype, imageloc from tbl_parkingspaces where lotid='"
						+ lotId + "'");
		...
		if (spacesList.size() > 0) {
			for (int i = 0; i < spacesList.size(); i++) {
				...
				ret += "['" + spacesList.get(i).get(0) + "', "
						+ spacesList.get(i).get(1) + ", "
						+ spacesList.get(i).get(2) + ", '" + lT + "', '"
						+ vDesc + "', '" + spacesList.get(i).get(4) + "'],";
			}
			ret = ret.substring(0, ret.length() - 1);
	...
	public String welcomeM(ModelMap model) {
		List<List<String>> retDb = objDatabaseConnect
				.runSqlQuery("select lat,lon,lotname,numid,lotid from tbl_parkinglots");
		String ret = "";
		if (retDb.size() > 0) {
			for (int i = 0; i < retDb.size(); i++) {
				ret += "['" + retDb.get(i).get(2) + "', " + retDb.get(i).get(0)
						+ ", " + retDb.get(i).get(1) + ", "
						+ retDb.get(i).get(3) + ", '" + retDb.get(i).get(4)
						+ "'],";
			}
			ret = ret.substring(0, ret.length() - 1);
			model.addAttribute("latlonpairs", ret);
